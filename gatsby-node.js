...
exports.createSchemaCustomization = async ({ actions }) => {
  ...
  actions.createTypes(/* GraphQL */ `
    ...
    interface HomepageABTest implements Node & HomepageBlock {
      id: ID!
      blocktype: String
      listName: String
      unfrmAbTest: ContentfulAbTestField
      variants: [Variant]
    }
  `)
  ...
  actions.createTypes(/* GraphQL */ `
    ...
    type ContentfulAbTestField {
      testId: String
      name: String
    }

    type ContentfulTestField {
      foo: String
    }
    ...
    interface VariantComponent implements Node {
      id: ID!
      blocktype: String
    }

    union VariantComponentUnion = ContentfulHomepageHero | ContentfulHomepageFeature | ContentfulHomepageCta | ContentfulHomepageBenefitList | ContentfulHomepageProductList | ContentfulHomepageLogoList

    type Variant implements Node {
      identifier: String
      distribution: Int
      sectionType: String
      component: VariantComponentUnion
    }

    type ContentfulAbTest implements Node & HomepageABTest & HomepageBlock {
      id: ID!
      blocktype: String @blocktype
      listName: String
      unfrmAbTest: ContentfulAbTestField
...
exports.createResolvers = ({ createResolvers }) => {
  const resolvers = {
    ContentfulAbTest: {
      unfrmAbTest: {
        resolve(source, args, context, info) {
          const unfrmAbTestLinkedNode = context.nodeModel.getNodeById({ id: source.unfrmAbTest___NODE });
          
          if (!unfrmAbTestLinkedNode) {
            console.warn('Linked node for unfrmAbTest is undefined:', source.unfrmAbTest___NODE);
            return null;
          }
      
          const jsonString = unfrmAbTestLinkedNode.internal.content;
          let originalContent = {};
      
          try {
            originalContent = JSON.parse(jsonString);
          } catch (error) {
            console.error('Error parsing JSON content in unfrmAbTestLinkedNode:', error);
          }
      
          return {
            testId: originalContent.id,
      ...
      variants: {
        resolve(source, args, context, info) {
          const variants = source.variants___NODE.map(variantId => {
            const variantNode = context.nodeModel.getNodeById({ id: variantId });
            
            if (!variantNode) {
              console.warn('Linked node for variant is undefined:', variantId);
              return null;
            }

            // Resolve the component node
            const componentNode = context.nodeModel.getNodeById({ id: variantNode.component___NODE });

            if (!componentNode) {
              console.warn('Component node for variant is undefined:', variantNode.component___NODE);
              return null;
            }

            return {
              identifier: variantNode.identifier,
              distribution: variantNode.distribution,
              sectionType: componentNode.internal.type.replace("Contentful", ""),
              component: {
                ...componentNode,
              }
            };
